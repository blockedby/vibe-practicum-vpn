...
## Review
- Correct: Existing Go CLI already separated `test` dry runs from `pick` production apply, parsed basic VLESS ws/reality links, and had package tests for core parser/picker/xray temp config behavior.
- Fixed: Hardened config loading/validation, JSON field tags, VLESS validation and reality/tls parameters, SOCKS reply parsing, subscription HTTP errors, state/result write errors, and xray apply rollback on failed restart; added focused tests and refreshed VLESS picker docs.
- Note: `go test ./...` passes locally.
