...
- `scripts/vpnkit/vpnkit-test-lab-setup.sh --endpoint 127.0.0.1 --port 21194`: PASS; generated ignored lab artifacts under `secrets/vpnkit-labs/steamdeck-host/` and printed only paths/metadata.
...
- `python3 scripts/vpnkit/vpnkit-manifest-validate.py --manifest config/vpnkit-manifest.example.yaml --server steamdeck --client host-machine --profile-intent test`: NOT RUN/PASS blocked locally by missing public Python dependency `jsonschema`.
