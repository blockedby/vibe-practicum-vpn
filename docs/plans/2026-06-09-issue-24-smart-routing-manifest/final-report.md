...
- AC2 validator/resolver: done. `scripts/vpnkit-manifest-validate.py` validates via PyYAML/jsonschema, performs semantic pair/capability/binding checks, emits sanitized JSON, and fails clearly when dependencies are missing.
- AC3 profile renderer: done. `scripts/vpnkit-render-profile-for-pair.sh` renders pair-specific fixture/real-mode profiles under ignored output, mode `600`, without profile/secret stdout; real mode fails clearly on missing local bindings.
