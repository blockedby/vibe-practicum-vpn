...
- Deployment helper added: `scripts/vpnkit/vpnkit-prod-singbox-dns-migration.sh` with verify/deploy modes, Docker-label workdir discovery, rollback backups, render/check without deprecated DNS env, vpnkit-only recreate, and runtime smoke.
...
- Final production deploy corrected bootstrap resolver to `direct-dns` with `type=local`, because direct DoT to 8.8.8.8:853 was not reliable on all endpoints. Final production verify passed on both endpoints with deprecated env absent.
- Final profile smoke passed for domain and two forced endpoints. VPN-over-VPN disposable smoke remains diagnostic-only: outer OpenVPN and sing-box startup pass without deprecated DNS env, but the client-container inner route did not fully capture via `sb-tun0`; DNS/hostname HTTPS failed while literal-IP HTTPS and ping passed. This is recorded as a follow-up diagnostic, not a production rollback blocker.
