...
usage() {
  cat <<'EOF'
Usage: scripts/vpnkit/vpnkit-test-lab-setup.sh [--endpoint HOST] [--port PORT]
...
VPNKIT_SECRETS_DIR="$BASE" VPNKIT_ROUTING_MODE="$ROUTING_MODE" VPNKIT_RULESET_SOURCE_MODE="${VPNKIT_RULESET_SOURCE_MODE:-local-fixture}" VPNKIT_SELECTED_OUTBOUND_MODE="${VPNKIT_SELECTED_OUTBOUND_MODE:-direct-fixture}" VPNKIT_OPENVPN_PUSH_DNS="${VPNKIT_OPENVPN_PUSH_DNS:-172.19.0.1}" scripts/vpnkit/vpnkit-render-local-configs.sh >/dev/null
