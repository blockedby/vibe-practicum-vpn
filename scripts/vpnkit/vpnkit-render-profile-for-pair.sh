...
usage() {
  cat <<'USAGE'
Usage: scripts/vpnkit/vpnkit-render-profile-for-pair.sh --server <id> --client <id> [options]
...
resolve_json=$(python3 scripts/vpnkit/vpnkit-manifest-validate.py --manifest "$MANIFEST" --server "$SERVER" --client "$CLIENT" --profile-intent "$PROFILE_INTENT")
