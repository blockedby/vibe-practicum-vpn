...
usage() {
  cat <<'EOF'
Usage: scripts/vpnkit/vpnkit-vibe-vpn-e2e.sh [options]
...
for path in \
  ...
  if [[ ! -r "$path" ]]; then
    ...
    echo "Run scripts/vpnkit/vpnkit-render-local-configs.sh after preparing gitignored secrets/vps inputs." >&2
    exit 1
  fi
done
if [[ ! -r secrets/vps/rendered/vibe-vpn/sub_url ]]; then
  ...
  echo "Create secrets/vps/vibe-vpn/sub_url (or subscription.url/subscription.txt) and rerun scripts/vpnkit/vpnkit-render-local-configs.sh." >&2
  exit 1
fi
...
COMPOSE_PROJECT_NAME="$PROJECT" scripts/vpnkit/vpnkit-collect-evidence.sh "$EVIDENCE" || true
