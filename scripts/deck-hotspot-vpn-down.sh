...
usage(){ cat <<'EOF'
Usage: scripts/deck-hotspot-vpn-down.sh [--ssh-target deck] [--connection NAME] [--nft-table NAME] [--report PATH]
...
[[ "$CONNECTION" =~ ^[A-Za-z0-9_.:-]+$ && "$NFT_TABLE" =~ ^[A-Za-z0-9_:-]+$ ]] || { echo "unsafe connection/table name" >&2; exit 2; }
...
redact(){ sed -E -e 's/([0-9]{1,3}\.){3}[0-9]{1,3}/<IP>/g' -e 's/([0-9a-f]{2}:){5}[0-9a-f]{2}/<MAC>/Ig' -e 's/[0-9a-f]{4}(:[0-9a-f]{0,4}){2,7}/<IPv6>/Ig' -e 's/[0-9a-f]{8}-[0-9a-f-]{27,}/<UUID>/Ig' -e 's/(Machine ID:).*/\1 <redacted>/I' -e 's/(Boot ID:).*/\1 <redacted>/I'; }
{
  ...
  echo
  echo '```text'
  ssh "${SSH_OPTS[@]}" "$SSH_TARGET" "CONNECTION='$CONNECTION' NFT_TABLE='$NFT_TABLE' bash -s" <<'REMOTE'
set -Eeuo pipefail
log(){ printf '[%s] %s\n' "$(date -u +%FT%TZ)" "$*"; }
log "down start"
