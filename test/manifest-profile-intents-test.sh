...
for intent in test production; do
  resolved=$(python3 scripts/vpnkit-manifest-validate.py --manifest "$MANIFEST" --server "$SERVER" --client "$CLIENT" --profile-intent "$intent")
...
render_out=$(scripts/vpnkit-render-profile-for-pair.sh --manifest "$MANIFEST" --server "$SERVER" --client "$CLIENT" --profile-intent test --out-dir "$OUT_DIR" --fixture)
